package com.bj58.fang.codeGenerate.detail;
...
//				if(variableName.equals("a_result_getListInfo_infolist21")) {
//					System.out.println();
//				}
//				if("UserID".equals(name) && "Long".equals(attrName) && !((String)value).endsWith("l")) {
//					System.out.println();
//				}
						...
						for(; ie < ((JSONArray)entry.getValue()).size(); ie++) {
							...
							for(int j = 0; j < yuan.size(); j++) {
								if(ttype.equals("Long")) {
									...
									arr1 +=  " new BigDecimal(" + yuan.get(j) + "),";
								}else {
									arr1 += yuan.get(j) + ",";
								}
...
//					typeImportSet.add(vClass.split(" ")[1]);
				...
				if(type.equals("Long")) {
					...
					for(int ie = 0; ie < ((JSONArray)entry.getValue()).size(); ie++) {
						arrStr += "new BigDecimal(" + ((JSONArray)entry.getValue()).get(ie) + "),";
					}
				}else {
					for(int ie = 0; ie < ((JSONArray)entry.getValue()).size(); ie++) {
						arrStr += ((JSONArray)entry.getValue()).get(ie) + ",";
